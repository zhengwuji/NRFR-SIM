...
- **触发条件**：推送到 `main` 分支的代码变更（修改 `*.md`、`docs/` 不会触发）
...
- compileSdk / targetSdk 升级到 36（Android 16），逐项验证隐藏 API 在新系统上签名未变
- AGP 8.13.2 · Kotlin 2.2.21 · Compose BOM 2026.06.01（material3 1.4.0）· Gradle 8.14.3 · JDK 21
- SIM 卡枚举改为 `activeModemCount` 动态探测，支持任意卡槽数，无需 READ_PHONE_STATE 权限
- Shizuku 配套升级：应用分发给 v13.6.0（首个支持 Android 16 QPR1 的版本），API 保持 13.1.5

**稳定性与体验**

- 所有系统服务调用移入 IO 协程，操作过程显示加载状态，写入后自动轮询确认生效
- 修复 Shizuku 监听器泄漏；Shizuku 服务被杀时自动回到未就绪界面
- 「无覆盖配置」与「读取失败」状态区分显示，失败时提示具体原因
- 新增英文界面（系统语言自动切换）

**桌面快速启动工具**

- adb 命令全部带超时保护，设备掉线不再卡死
- 只回收本程序启动的 adb server，不影响其他工具的 adb 会话
- Shizuku 启动改为轮询服务就绪，兼容 v13.6 新的启动脚本路径
- 版本号与 Android 端单源统一维护，不再三处硬编码
- 修复 Windows 控制台黑窗、生产版本误开调试器问题

**CI / 发布**

- 每次代码变更自动编译 release APK 并发布到 Releases（中文更新说明），README 等文档变更不触发
- release 构建自动回退 debug 签名，保证任何环境产出可安装的 APK
- Shizuku APK 固定版本 + SHA-256 校验，保证分发内容可复现

### v1.0.x（历史版本）

- 基础功能：SIM 卡国家码与运营商名称修改、还原
- 双卡支持、运营商预设列表、自定义国家码 / 运营商名
- 桌面快速启动工具（自动安装 Shizuku 与应用）
